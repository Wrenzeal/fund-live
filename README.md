...
- 基金目录可用状态治理已落地：
  ...
  - `cmd/crawler --list all --save-db --catalog-only` 现在会把名称含“(后端)”的份额标记为 `unavailable`，并在全量无 `--limit` 同步时把本地历史残留基金标记为 `catalog_missing`
  - 默认基金搜索只返回 `active`；历史持仓、自选和直接按 ID 查询仍可读取非 active 记录，避免因目录治理破坏既有数据
  - 2026-06-01 已重新执行全量目录同步并清理本地冗余：先标记 `catalog_missing=111`，确认无用户自选/持仓/交易流水/估值档案引用后已备份并删除；当前本地 `funds` 表 26,927 条，其中 `active=26,737`、`unavailable=190`、`catalog_missing=0`，`stock_holdings` 为 4,533 条

- 基金人工分类标签覆盖层已落地：
  - 后端 `fund_classification_overrides` 已扩展主主题与人工标签字段，并新增管理员接口读取分类字典、读取/保存单只基金人工覆盖
  - Dashboard 会返回 `classification_override`；首页与量化详情页的持仓分类卡可显示“人工校正”、人工标签、备注和管理员编辑入口
  - 人工主分类 / 主板块 / 主主题只作为展示与分析输入的覆盖层；下方行业/主题 Top 权重仍保留自动持仓聚合结果，避免人工标签被误读为真实持仓占比
  - 保存人工分类后会失效该基金旧量化快照，刷新后按新的有效分类重建分析

- 前端主要页面已统一接入滚动进入动效：
  - 新增共享 `scroll-reveal.tsx`，提供 `ScrollReveal` / `ScrollRevealStack` / `useLazyReveal`，基于原生 `IntersectionObserver` 与 CSS transition，不新增动画依赖
  - 首页、持仓、自选、想法、公告、登录/注册、VIP 介绍/开通/任务/报告、公告详情、想法详情、持仓流水详情均已按页面区块或列表卡片接入滚动渐显
  - 量化详情页与排行榜继续通过 `analysis-layout.tsx` 复用同一 reveal 能力，并保留 `motion-reduce` 降级

- 量化看板与排行榜前端布局已再次重构：
  - `/analysis/[fundId]` 现按“首屏结论 → 洞察条 → 建议分布/六维模块 → 主反证据/AI边界 → 事件/风险 → 结构/季报 → 数据口径/持仓 → 方法限制”组织
  - 删除旧版重复概览和重复方法说明，把数据口径、方法限制、结构暴露收敛到独立压缩模块
  - 新增滚动进入渐显与建议分布滚动绘制动效，不新增前端依赖
  - `/analysis/rankings` 现按“首屏概览 → 三类观察池统计 → 首位样本放大 → 分榜列表 → 排行榜口径”组织；上方积极池/观察池/风险池统计卡保留并列展示，下方“结构偏积极 / 最值得观察 / 高风险关注”分榜改为上下排布，避免宽屏并列导致阅读割裂
  - 新增共享 `analysis-layout.tsx`，量化详情页和排行榜共用滚动 reveal 与 section heading
...
- 持仓页体验增强：
  - 持仓页已从长页面堆叠改为工作台导航：`总览 / 记录 / 持仓 / 风险 / 流水 / 工具` 六个功能块，顶部提供“记一笔 / 看持仓 / 查风险 / 看流水”快捷按钮；各块只保留短标题和就近操作，降低文字噪音
...
- 基金量化分析基础版量化看板已可用：
  - `dashboard` 主链路直接返回 `analysis`
  - 首页已有量化分析摘要卡
  - 独立详情页 `/analysis/[fundId]` 已落地
  - 第一版已覆盖综合评分、加 / 平 / 减分布、六维模块评分、事件分析、主要理由与风险提示
- 量化看板事件层已升级为“当前持仓优先”：
  - 事件层主轴优先围绕“当前持仓 + 持仓股票当前表现 / 相关时事”
  - 上一季持仓变化仅保留为辅助观察，不作为主结论核心
- 量化看板现已接入**当前重仓股公告事件源**：
  - 通过 CNINFO 官方公告查询页按需抓取前几大重仓股近 45 天公告
  - 第一版纳入：业绩预告、定期报告、经营类公告、公司治理类公告
  - 这些事件已进入 `analysis.event_impacts`，并同步参与 `summary / reasons / warnings`
- 量化看板现已接入**基金自身公告事件源**：
  - 通过 Eastmoney `api.fund.eastmoney.com/f10/JJGG` 按需抓取基金公告
  - 第一版纳入：定期报告、基金销售、人事调整与其他公告
  - 这些事件以 `target_scope=fund` 进入量化事件层，并同步参与 `summary / reasons / warnings`
- 量化看板现已支持**板块 / 主题时事轻量聚合**：
  - 基于当前重仓股近期公告事件，聚合出主线级事件提示
  - 第一版会生成类似“半导体芯片主线近期事件密集”的事件卡
  - 当前仍属于轻量聚合，不是独立板块资讯源
- 对联接基金 / 目标 ETF 口径的量化分析，量化看板也会同步读取**目标 ETF 自身公告事件**，避免事件层只停留在联接基金壳层
- 量化看板现已补全第一版**行业 / 主题权重变化与风格漂移提示**：
  - 会输出行业 / 主题权重提升与回落事件
  - 当行业 / 主题权重出现双向显著变化时，会提示当前组合存在风格漂移迹象
- 量化看板上一季持仓现已支持**本地持久化历史快照**：
  - 新增 `stock_holding_history`
  - 读取上一季持仓时优先使用本地历史快照
  - 若本地缺失，再按需抓取 Eastmoney 年度持仓页并落库
  - 对联接基金 / 目标 ETF 口径，也会优先按目标 ETF 代码对齐上一季历史持仓
- 量化看板现已接入第一版**宏观 / 政策事件**：
  - 通过轻量宏观事件种子与行业/主题映射，把近期官方宏观/政策事件接入量化事件层
  - 这些事件会以 `target_scope=macro` 出现在 `analysis.event_impacts`
  - 当前仍属于第一版轻量方案，不是实时宏观新闻流
- 量化看板现已接入第一版**纯指数层事件**：
  - 当前会输出指数调样 / 样本维护窗口提示
  - 这些事件会以 `target_scope=index` 出现在 `analysis.event_impacts`
  - 第一版主要覆盖“调样窗口临近”提示，后续再扩展到更细的指数规则变化与调样结果
- 量化看板已完成第一轮**权重 / 阈值校准**：
  - 对强趋势 ETF / ETF联接样本，降低了风险 / 性价比模块的过度压制
  - “偏加仓”结论阈值已从 `increase>=60` 前移到 `>=55`
  - 运行态样本复核中，`159813` 从 `62.2` 提升到 `68.1`，`012970` 从 `64.6` 提升到 `70.4`
- 六个核心样本已完成第一轮**逐只验收**：
  - `159813 / 012970 / 023408 / 005827 / 000362 / 000370`
  - 当前代码版后端审计结果已全部通过首轮验收
  - 第二轮校准还移除了“中性主线事件对事件分的隐性抬升”，避免中性事件把总分悄悄抬高
- 持仓页现已接入第一版**量化分析标签**：
  - 在按基金聚合卡片与分笔明细行内显示“偏加仓 / 偏持有 / 偏减仓”标签
  - 同时补充风险标签与可选总分，方便在持仓页快速扫一遍组合
- 量化看板也支持第一版季度辅助事件：
  - 前十大换手变化
  - 单只重仓股季度内权重提升 / 回落
  - 前三大集中度变化
...
- 分类主类选择逻辑已调整：
  ...
  - “其他权益 / 其他主题”已更名为“未归类权益 / 未归类主题”
- 分类置信度改为按“已识别覆盖率”判断，不再单纯按总持仓覆盖率给高置信度
- 主题分类新增“按行业回退到主题”的 fallback，像半导体 ETF 这类基金现在可以落到更合理的“半导体芯片”等主题
- 基金主页分类模块新增“覆盖有限”提示，便于识别低置信度分类结果

## 2026.4.24 更新摘要

- ETF 联接基金主展示口径已调整：对于已解析出目标 ETF 的联接基金，页面主展示的实时涨跌幅现优先跟踪**目标 ETF 的实时涨跌幅**
- 当当前默认行情源拿不到目标 ETF quote 时，系统现会自动尝试其它已注册国内行情源；只有全部 ETF quote source 都失败时，才回退到持仓估值
- 修复联接基金“追踪目标”模块权重显示为 `0.00%` 的问题：主追踪目标现默认显示为 `100.00%`
- 当可获取目标 ETF 的持仓时，基金主页会在“追踪目标”模块下方新增一个“目标 ETF 持仓”模块，展示该 ETF 的持仓明细
- 自选页分组管理进一步补全：支持弹窗编辑分组名称 / 分组说明 / 分组颜色，并支持拖拽排序且刷新后保持顺序
- 新增联接基金残留直持仓治理工具：`cmd/feeder-holdings-cleanup`
  - 支持扫描 residual feeder holdings
  - 自动生成白名单 CSV
  - 自动导出备份 CSV
  - 提供默认 dry-run 的 cleanup 模式

## 2026.4.23 更新摘要

- 修复自选页“分组导航”跳转定位偏移问题：点击分组导航胶囊后，目标分组不再被 sticky 的“你的自选”头部区域遮挡
- `AccountAreaShell` 现会提供动态锚点偏移量：桌面端按完整 sticky 头部高度计算，移动端按折叠后的顶部栏高度计算
- 自选页分组 section 已切到动态 `scroll-margin-top`，跳转后无需再手动补滚
- 自选页“分组导航”胶囊已补充 hover / click 动效，当前浏览分组会有更明显的高亮与反馈
- 每个分组头部的“展开/收起分组”按钮已增加平移动效、流光反馈和箭头旋转动画
- 自选页新增“当前浏览分组”提示卡片，并在浏览模式说明中直接显示当前分组名称，降低“当前分组”语义不清的问题
- 自选页分组现已支持完整管理增强：
  - 支持通过弹窗直接编辑“分组名称 + 分组说明 + 分组颜色”
  - 支持拖拽分组卡片调整顺序（在“全部分组”且未搜索时可用）
  - 后端会持久化分组顺序，刷新后保持一致
- “我的自选”、持仓页、VIP 页等用户可见文案已统一收口，去掉了偏开发/偏 demo 的展示口吻

## 2026.4.22 更新摘要

- 持仓页现已支持“实时盈亏预估”：白天按基金预估涨跌幅展示预估盈亏，夜间官方真实涨跌同步后会自动覆盖预估展示
- 单条持仓、按基金聚合视图和顶部总览卡片都已统一到“预估 -> 官方覆盖”的逻辑
- 基金主页“持仓分类”不再把板块和主题混在一起，现已拆成横向并列的两个子模块：
  - `行业板块`
  - `主题分类`
- 每个子模块内部继续以列表形式展示主项与 Top 占比，更方便区分行业暴露与主题暴露
- 自选页现已支持分组搜索、顶部快速跳转、分组折叠，以及 `全部分组 / 当前分组` 聚焦浏览

## 2026.4.21 更新摘要

- 首页基金详情在集合竞价阶段不再整块隐藏持仓模块，持仓明细、重仓股覆盖、持仓占比与 TOP 卡片会继续展示静态持仓信息
- 集合竞价阶段的现价、涨跌幅、贡献等动态字段改为占位，等待开盘后恢复
- 联接 / 跟踪基金详情默认只展示下一层目标（如目标 ETF / 跟踪指数），不再直接展开到底层股票
- `GET /api/v1/fund/:id/holdings` 已新增 `display_level`、`display_items` 与 `lookthrough_available`
- 基金主页“持仓明细”与右侧统计 / TOP 卡片已适配 `target_layer`
- 已新增 `fund_estimate_capabilities`，后台会扫描 direct holdings / feeder target / valuation profile / QDII holdings 等链路，识别当前可实时估值基金
- 后台估值覆盖调度器现会把 `supported / degraded` 基金纳入更新池，不再只依赖“谁被访问、谁进入 collector”
- 实时估值 collector 已改为按 batch + goroutine 并发执行，并区分 request-driven 高频基金与 capability-driven 全量基金
- 基金分类已升级为“行业 + 主题”双层体系，新增 `AI应用`、`算力`、`CPO/光模块`、`商业航天`、`卫星互联网`、`机器人`、`数据基础设施`、`军工电子`
- 基金主页 `dashboard` 现会同时返回 `sector_snapshot` 与 `theme_snapshot`，页面会同时展示主板块与主主题

## 2026.4.19 更新摘要

- 基金分类模块已推进到第二阶段：新增稳定主分类层，并保留持仓推导的板块快照层
- 基金主页现可展示主分类、主板块与 Top 3 板块
- 搜索接口现支持按 `category` / `sector` 过滤，搜索结果会一并返回主分类信息
- 普通基金、联接基金、QDII 三类基金当前都可参与板块快照生成
- 持仓页总仓现可展示“已就绪部分”的官方汇总，并明确提示已覆盖的持仓条数与本金范围
- 单条持仓在真实净值未就绪时，会优先按确认份额展示盘中预估市值 / 盈亏；份额未确认时则只保留按本金口径的提示性预估
- 持仓页文案现已明确区分“最新官方市值 / 今日官方盈亏”和“盘中预估市值 / 盘中预估盈亏”
- 持仓页现新增 `按基金 / 分笔明细` 与 `官方口径 / 盘中预估` 双切换，默认视图为 `按基金 + 官方口径`
- 同一基金的多笔持仓现可按基金聚合展示，并支持展开查看分笔
- 盘中预估总览与聚合卡片只汇总已确认份额的记录，未确认份额不会被混入预估总值
...
- 持仓页顶部新增总本金 / 总价值 / 总收益 / 总今日盈亏 / 总今日涨跌幅总览卡
...
如果只是刷新基金搜索/目录覆盖范围，优先使用目录同步模式；它只导入全量基金 code/name/type 和目录可用状态，不逐只抓详情和持仓，也不会覆盖已有经理、公司、净值、规模和持仓。全量 `--list all` 且不带 `--limit` 时，会把上游目录中名称含“(后端)”的不可用份额标记为 `unavailable`，并把本地历史残留基金标记为 `catalog_missing`；默认搜索只返回 `active`：
...
# 全量详情与持仓导入，耗时很长，建议先用 --catalog-only 刷目录，再按需分批补详情/持仓
...
- 全量基金目录可用 `--catalog-only` 快速刷新；默认搜索会过滤 `unavailable / catalog_missing` 目录状态，确认无用户数据引用后可清理 `catalog_missing` 冗余记录；这不等于已抓取详情/持仓，全量基金详情与持仓抓取耗时较长，不建议一次性在弱网络环境下跑完
