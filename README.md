...
- 运行时配置语义统一为以 `fundlive.yaml` 为主，新增 `database.log_level` / `database.auto_migrate`，当前推荐 `warn + 关闭自动迁移`
- 基金详情、持仓、估值与联接基金解析改为共享只读瞬时补全缓存，GET 请求不再触发数据库写入
- `/api/v1/fund/:id` 与 `/api/v1/fund/:id/holdings` 在目录库数据缺失时会做只读瞬时补全，联接基金持仓接口也会与估值链路统一到目标 ETF fallback
- 联接基金失败映射新增冷却重试窗口，避免每次请求都重复打外部搜索
- 夜间官方净值同步启动时会先检查当日数据是否已齐，避免 `23:00` 后重启立即重复补跑
- CORS 改为显式 allowlist，支持带凭据的安全跨域访问
...
通常是数据库里只有基金目录、没有详情和持仓。当前后端已支持按需补抓，重试一次即可；如果仍失败，检查后端日志和外部数据源可达性。
