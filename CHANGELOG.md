...
- **基金搜索聚焦体验重构**
  ...
  - 首页 Header 的常态搜索区域放宽，缓解被品牌、导航和账户控件挤压导致内容看不清的问题。
  - 常态搜索入口进一步收口为图标按钮，避免挤压市场倒计时、账户与模式控件；弹层补齐 dark / classic / cyber 三主题触发器、蒙版和面板质感，并加入柔和退场动画。

- **量化评分仪表盘可读性优化**
  - 放大评分图中心读数区、总分数字、`/100` 和状态标签，降低小尺寸 summary 卡片里内部文字过小的问题。
  - 降低环形刻度和内圈虚线的视觉干扰，并增强中心底色对比，使深色、Classic 浅色和移动端下读数更清楚。
  - 保留原有评分算法、动画触发、ARIA 描述和 `hero` / `summary` 调用方式不变。

- **首页量化概览评分卡片重排**
  - 基金页面的量化概览模块改为评分仪表盘优先展示，评分图独立置于卡片上方视觉焦点区。
  - 结论摘要、主方向、风险等级、证据数量、建议分布和原因卡片改为下方分层承接，减少原评分容器下半部分空白。
  - 保留事件雷达和完整量化看板入口，未改动量化接口、评分规则或投资建议口径。
