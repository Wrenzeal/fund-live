...
import { CalendarDays, CheckCircle2, Clock3, History, LoaderCircle, Plus, Sparkles, Tags, Wallet } from 'lucide-react'
...
export function HoldingRecordComposer({
  ...
  const steps = [
    { label: '选基金', done: hasFund, detail: hasFund ? selectedFundLabel : '代码或名称都可以' },
    { label: '填金额', done: hasAmount, detail: hasAmount ? `¥${amountValue.toLocaleString('zh-CN')}` : '可以先填大概本金' },
    { label: '定日期', done: hasTradeDate, detail: hasTradeDate ? `${tradeDate} · ${tradeTimingLabel}` : '影响确认净值日' },
  ]
    ...
    <section className="mb-6 overflow-hidden rounded-[34px] border border-cyan-400/18 bg-[var(--card-bg)]/86 p-5 shadow-[0_22px_54px_rgba(2,8,23,0.10)] glass">
      <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <h2 className="mt-4 text-3xl font-black text-theme-primary">先把仓位记下来，后面自动算</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-theme-secondary">
            只需要基金、金额和交易日。系统会自动推导确认净值日，后续同步官方净值、份额、真实盈亏和量化风险。
          </p>
        </div>
          ...
          {steps.map((step, index) => (
            ...
            >
              <div className="flex items-center justify-between gap-2">
                <span className="text-xs font-medium text-theme-primary">{index + 1}. {step.label}</span>
                {step.done ? (
              ...
              <div className="mt-1 truncate text-[11px] text-theme-muted">{step.detail}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid gap-5 xl:grid-cols-[1.08fr_0.92fr]">
        <div className="space-y-4">
          {visibleRecentAggregates.length > 0 && (
            <div className="rounded-[28px] border border-amber-400/18 bg-amber-400/8 p-4">
              <div className="flex items-start justify-between gap-3">
                ...
                <span className="rounded-full border border-amber-400/25 bg-amber-400/10 px-3 py-1 text-[11px] tracking-[0.18em] text-amber-200">复记</span>
              </div>
              <div className="mt-3 flex gap-2 overflow-x-auto pb-1">
                {visibleRecentAggregates.map((aggregate) => {
                  ...
                  return (
                    ...
                    >
                      <div className="truncate text-sm font-semibold text-theme-primary">{aggregate.fund?.name || aggregate.fund_id}</div>
                      <div className="mt-1 text-xs text-theme-muted">{aggregate.holding_count} 笔 · 本金 {formatInlineMoney(aggregate.total_principal)}</div>
                    </button>
                  )
                })}
              </div>
          ...
          <div className="rounded-[28px] border border-[var(--card-border)] bg-[var(--input-bg)]/58 p-4">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start">
              <label className="min-w-0 flex-1 space-y-2">
                <span className="text-sm font-medium text-theme-primary">你在记录哪只基金？</span>
                <input
                  value={query}
            ...
            <div className="mt-4 grid gap-2 md:grid-cols-2 xl:grid-cols-3">
                ...
                return (
                  ...
                  >
                    <span className="min-w-0">
                      <span className="block truncate text-sm font-medium text-theme-primary">{fund.name}</span>
                      <span className="mt-1 block text-xs text-theme-muted">{fund.id}</span>
                    </span>
                    {active ? <CheckCircle2 className="h-4 w-4 shrink-0 text-cyan-300" /> : <Plus className="h-4 w-4 shrink-0 text-cyan-300" />}
                  </button>
                )
              })}
              ...
              {results.length === 0 && (
                <div className="rounded-2xl border border-dashed border-[var(--card-border)] px-4 py-8 text-center text-sm text-theme-secondary md:col-span-2 xl:col-span-3">
                  输入至少 2 个字符后展示候选基金；如果基金唯一匹配，也会自动识别。
                </div>
              )}
            </div>
          </div>
          ...
          <div className="grid gap-4 lg:grid-cols-[0.95fr_1.05fr]">
            <div className="rounded-[28px] border border-[var(--card-border)] bg-[var(--input-bg)]/58 p-4">
              <label className="space-y-2">
                <span className="text-sm font-medium text-theme-primary">这笔大概多少本金？</span>
                <input
                  value={amount}
            ...
            <div className="rounded-[28px] border border-[var(--card-border)] bg-[var(--input-bg)]/58 p-4">
              <label>
                <span className="text-sm font-medium text-theme-primary">给未来的自己留一句备注</span>
                <input
                  value={note}
              ...
              <span className="mt-3 block text-xs leading-5 text-theme-muted">备注可选，但建议写下这笔钱的策略背景，回看时更有价值。</span>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-[30px] border border-[var(--card-border)] bg-[var(--card-bg)]/88 p-5">
          <div className="flex items-start justify-between gap-3">
            <div>
              <div className="text-sm font-medium text-theme-primary">交易时间与确认规则</div>
              <div className="mt-1 text-xs leading-5 text-theme-muted">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {['今天', '上个交易日', '下个交易日'].map((shortcut) => {
              const shortcutDate = shortcut === '今天'
                ? todayTradeDate
                : shortcut === '上个交易日'
                  ? previousTradeDate
                  : nextTradeDate
          ...
          <div className="mt-4 grid gap-3 rounded-[24px] border border-[var(--card-border)] bg-[var(--input-bg)]/60 px-4 py-4 text-sm">
            <div className="text-[11px] font-medium tracking-[0.18em] text-theme-muted">本次将记录</div>
            <div className="grid gap-3 sm:grid-cols-3">
              <div>
                <div className="text-xs text-theme-muted">基金</div>
                <div className="mt-1 truncate font-semibold text-theme-primary">{selectedFundLabel}</div>
              </div>
              <div>
                <div className="text-xs text-theme-muted">本金</div>
                <div className="mt-1 font-semibold text-theme-primary">{hasAmount ? formatInlineMoney(String(amountValue)) : '--'}</div>
              </div>
              <div>
                <div className="text-xs text-theme-muted">备注</div>
                <div className="mt-1 truncate font-semibold text-theme-primary">{note || '未填写'}</div>
              </div>
            </div>
          </div>
          ...
          >
            ...
            <span className="relative z-10">{isAddingHolding ? '提交中...' : canSubmit ? '记录这笔持仓' : remainingHint}</span>
          </button>
        </div>
      </div>
    </section>
