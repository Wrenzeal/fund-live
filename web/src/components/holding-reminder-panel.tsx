...
function loadReminderState(): ReminderState {
  ...
  try {
    ...
    const snoozed = Object.fromEntries(
      Object.entries(parsed.snoozed ?? {}).filter(([, expiresAt]) => typeof expiresAt === 'number' && expiresAt > now)
    )
    return {
...
function toneMeta(tone: InsightTone) {
  switch (tone) {
    case 'danger':
      return { className: 'border-rose-400/28 bg-rose-500/10 text-rose-100', icon: ShieldAlert }
    case 'warning':
      return { className: 'border-amber-400/28 bg-amber-400/10 text-amber-100', icon: AlertTriangle }
    case 'good':
      return { className: 'border-emerald-400/24 bg-emerald-400/10 text-emerald-100', icon: CheckCircle2 }
    case 'info':
    default:
      return { className: 'border-cyan-400/24 bg-cyan-400/10 text-cyan-100', icon: Info }
  }
}
...
export function HoldingReminderPanel({
  ...
  const [reminderState, setReminderState] = useState<ReminderState>(() => loadReminderState())
  ...
  const visibleReminders = useMemo(
    () => summary.reminders.filter((reminder) => (
  ...
  const visibleUrgentCount = visibleReminders.filter((reminder) => reminder.tone === 'danger' || reminder.tone === 'warning').length

  const updateReminderState = (updater: (current: ReminderState) => ReminderState) => {
    ...
    <section className="mb-6 overflow-hidden rounded-[30px] border border-[var(--card-border)] bg-[var(--card-bg)]/84 p-5 glass">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <h3 className="mt-3 text-2xl font-black text-theme-primary">关键变化先提醒</h3>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-theme-secondary">
            页面内生成提醒并支持已读 / 稍后提醒：官方口径待补齐、单只跌幅、量化风险升高、校正长期未复核、重仓主题事件信号。
          </p>
          {hiddenCount > 0 && (
        ...
        <div className="rounded-[24px] border border-rose-300/20 bg-rose-400/10 px-4 py-3 text-center">
          ...
          <div className="mt-1 text-3xl font-black text-theme-primary">{visibleUrgentCount}</div>
        </div>
      </div>
        ...
        {visibleReminders.length === 0 ? (
          <div className="rounded-[24px] border border-emerald-400/22 bg-emerald-400/10 p-4 text-sm text-emerald-100 md:col-span-2 xl:col-span-3">
            <div className="flex items-center gap-2 font-semibold">
              ...
              {summary.reminders.length === 0 ? '暂无需要提醒的异常项' : '当前提醒已处理'}
            </div>
          ...
          return (
            <div key={reminder.id} className={cn('rounded-[24px] border p-4', meta.className)}>
              <div className="flex items-start justify-between gap-3">
                <div className="flex min-w-0 items-center gap-2">
                  <Icon className="h-4 w-4 shrink-0" />
                  <div className="truncate text-sm font-semibold text-theme-primary">{reminder.title}</div>
                </div>
                {reminder.metric && <span className="shrink-0 rounded-full border border-white/15 bg-white/8 px-2 py-0.5 text-[11px]">{reminder.metric}</span>}
              </div>
              <p className="mt-2 text-xs leading-5 text-theme-secondary">{reminder.description}</p>
              {reminder.action && <div className="mt-3 text-xs font-medium text-theme-primary">{reminder.action}</div>}
