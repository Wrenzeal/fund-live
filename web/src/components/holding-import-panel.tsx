...
interface HoldingImportPanelProps {
  ...
  onImportBatch?: (
    items: CreateHoldingBatchItem[],
  ) =>
    | Promise<HoldingBatchCreateResult | null>
    | HoldingBatchCreateResult
    | null
    | void
}
...
function parseImportText(
  ...
  const lines = raw
    ...
    .filter(Boolean)
  const dataLines =
    lines[0]?.includes('基金代码') || lines[0]?.toLowerCase().includes('fund')
      ? lines.slice(1)
      : lines
...
export function HoldingImportPanel({
  recentAggregates,
  compact = false,
  onSelectDraft,
  onImportBatch,
}: HoldingImportPanelProps) {
  const [rawText, setRawText] = useState('')
  const [source, setSource] = useState<HoldingSourceFilter>('alipay')
  const [showTemplate, setShowTemplate] = useState(false)
  const [isImporting, setIsImporting] = useState(false)
  const [importResult, setImportResult] =
    useState<HoldingBatchCreateResult | null>(null)
  const previewRows = useMemo(
    () => parseImportText(rawText, source),
    [rawText, source],
  )
  ...
  const canImport = Boolean(
    onImportBatch && validRows.length > 0 && !isImporting,
  )
  ...
  const handleImportBatch = async () => {
    ...
    try {
      const result = await onImportBatch(
        validRows.map<CreateHoldingBatchItem>((row) => ({
          fund_id: row.fundID,
          amount: row.amount,
        ...
        <div className="space-y-3 rounded-[24px] border border-[var(--card-border)] bg-[var(--input-bg)]/48 p-4">
          <label className="grid gap-2 text-xs font-medium text-theme-muted">
            ...
            <select
              value={source}
              onChange={(event) =>
                setSource(event.target.value as HoldingSourceFilter)
              }
        ...
        <div className="rounded-[24px] border border-[var(--card-border)] bg-[var(--input-bg)]/48 p-4">
            ...
            <div className="mt-4 rounded-[20px] border border-emerald-300/18 bg-emerald-400/10 px-3 py-3 text-xs leading-5 text-theme-secondary">
              <div className="font-semibold text-theme-primary">
                ...
                {importResult.failed_count > 0
                  ? `，${importResult.failed_count} 行未导入`
                  : ''}
              </div>
              {importResult.failed && importResult.failed.length > 0 && (
                <div className="mt-2 space-y-1">
                  {importResult.failed.slice(0, 5).map((failure) => (
                    <div
                      key={`${failure.index}-${failure.fund_id}-${failure.code}`}
                      className="text-amber-100"
                    >
                      第 {failure.index + 1} 行 {failure.fund_id || ''}：
                      {failure.message}
                    </div>
                  ))}
                </div>
          ...
          <div className="mt-4 space-y-2">
            {previewRows.length === 0 ? (
              ...
              previewRows.map((row) => (
                <div
                  key={`${row.index}-${row.fundID}-${row.amount}`}
                  className="rounded-[20px] border border-[var(--card-border)] bg-[var(--card-bg)]/58 p-3"
                >
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <div className="min-w-0">
                      <div className="text-sm font-semibold text-theme-primary">
                        第 {row.index} 行 · {row.fundID || '未识别基金'}
                      </div>
                      <div className="mt-1 text-xs text-theme-muted">
                        金额 {row.amount || '--'} · 日期{' '}
                        {row.tradeDate || '待补'} · {row.note || '无备注'}
                      </div>
                    </div>
                    {row.valid ? (
          ...
          {showTemplate && commonFunds.length > 0 && (
            <div className="mt-4 rounded-[20px] border border-cyan-300/18 bg-cyan-400/8 px-3 py-3 text-xs text-theme-secondary">
              ...
              常用基金：
              {commonFunds
                .map((item) => item.fund?.name || item.fund_id)
                .join(' / ')}
              。批量写入前请确认交易日期；不确定的行仍可带入上方逐笔补录。
            </div>
          )}
        </div>
      </div>
