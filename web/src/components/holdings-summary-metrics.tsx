...
export function HoldingsSummaryMetrics({
  ...
  const activeProfit = shouldUseOfficialSummary ? holdingSummary.total_today_profit : previewSummary.total_today_profit
  ...
  const metrics = [
    ...
    },
    {
      ...
      tone: metricTone(activeReady, activeProfit),
      note: shouldUseOfficialSummary
        ? (hasOfficialSummaryMetrics
  ...
  return (
    <div className="mb-6 grid gap-4 lg:grid-cols-4" data-metric-scope={metricScope}>
      {metrics.map((metric) => (
