...
export default function WatchlistPage() {
  ...
  const [localGroupOrderIds, setLocalGroupOrderIds] = useState<string[] | null>(null)
  ...
  const orderedWatchlistGroups = useMemo(() => {
    if (!localGroupOrderIds || localGroupOrderIds.length !== watchlistGroups.length) {
      return watchlistGroups
    }

    const groupByID = new Map(watchlistGroups.map((group) => [group.id, group]))
  ...
  const filteredGroups = useMemo(() => {
    ...
    return orderedWatchlistGroups.filter((group) => {
      ...
      return name.includes(normalizedGroupSearch) || description.includes(normalizedGroupSearch)
    })
  ...
  useEffect(() => {
    ...
    if (selectedGroupID && !watchlistGroups.some((group) => group.id === selectedGroupID)) {
      setSelectedGroupID('')
    }

    if (!focusedGroupId || !watchlistGroups.some((group) => group.id === focusedGroupId)) {
  ...
  useEffect(() => {
    if (editingGroupId && !watchlistGroups.some((group) => group.id === editingGroupId)) {
  ...
  useEffect(() => {
    ...
    if (
      ...
      watchlistGroups.every((group, index) => group.id === localGroupOrderIds[index])
    ) {
  ...
  const reorderEnabled = groupViewMode === 'all' && normalizedGroupSearch === '' && orderedWatchlistGroups.length > 1
  ...
  const triggerNavAnimation = (groupID: string) => {
    ...
    navAnimationTimerRef.current = window.setTimeout(() => {
      setAnimatingNavGroupId((current) => current === groupID ? null : current)
  ...
  const handleUpdateGroup = async (group: WatchlistGroup) => {
    ...
    try {
      await updateGroup(group.id, normalizedName, normalizedDescription, editingGroupAccent)
      ...
      setWatchlistFeedback({
        type: 'error',
        message: error instanceof Error ? error.message : '更新分组信息失败，请稍后重试。',
      })
      setSavingGroupId(null)
    }
  }
  ...
  const handleReorderGroups = async (nextOrderIds: string[]) => {
    ...
    try {
      ...
      setWatchlistFeedback({
        type: 'error',
        message: error instanceof Error ? error.message : '调整分组顺序失败，请稍后重试。',
      })
    } finally {
  ...
  const handleGroupDragOver = (event: DragEvent<HTMLElement>, targetGroupID: string) => {
    if (!reorderEnabled || !draggingGroupId || draggingGroupId === targetGroupID) {
      return
    }
              ...
              <section className="rounded-[32px] border border-[var(--card-border)] p-6 glass">
                <div className="grid gap-4 xl:grid-cols-[1.1fr_1fr]">
                  <div className="space-y-3">
                    ...
                    <div className="text-xs text-theme-secondary">
                      当前命中 {filteredGroups.length} / {watchlistGroups.length} 个分组
                    </div>
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                      {([
                        ...
                        },
                        {
                          ...
                          helper: focusedGroup ? `当前聚焦：${focusedGroup.name}` : '先选择一个分组',
                        },
                      ] as const).map((option) => {
                        ...
                        <button
                          ...
                          className={cn(
                            ...
                            animatingViewMode === option.id && 'action-button-pop'
                          )}
                        >
                          <span className="action-button-shine" />
                          <div className="relative z-10 flex items-start justify-between gap-3">
                            <div className="min-w-0">
                              <div className="text-[11px] tracking-[0.24em] text-theme-muted">{option.eyebrow}</div>
                              <div className="mt-2 flex items-center gap-2">
                                <Icon
                                  className={cn(
                                    'h-4 w-4 transition-transform duration-300',
                                    active ? 'scale-110 text-cyan-200' : 'group-hover:scale-110 group-hover:text-cyan-200'
                                  )}
                                />
                          ...
                          <div className="relative z-10 mt-4 space-y-1">
                            <div className="text-sm font-medium text-theme-primary">{option.description}</div>
                            <div className="text-xs leading-5 text-theme-secondary">{option.helper}</div>
                          </div>
                        </button>
                        )
                      })}
                ...
                {focusedGroup && (
                  <div className="mt-5 rounded-[24px] border border-cyan-500/25 bg-cyan-500/10 px-4 py-4 shadow-[0_16px_30px_rgba(34,211,238,0.10)]">
                    <div className="text-[11px] tracking-[0.24em] text-cyan-300">当前浏览分组</div>
                    <div className="mt-2 flex flex-wrap items-center gap-2">
                      <span className="rounded-full border border-cyan-400/35 bg-cyan-400/10 px-2.5 py-1 text-[11px] tracking-[0.18em] text-cyan-200">
                        {groupViewMode === 'focused' ? '当前仅展示' : '当前高亮'}
                      </span>
                      <span className="text-lg font-bold text-theme-primary sm:text-xl">{focusedGroup.name}</span>
                    </div>
                        ...
                        return (
                          <button
                            ...
                            className={cn(
                              ...
                              animatingNavGroupId === group.id && 'action-button-pop'
                            )}
                          >
                            <span className="action-button-shine" />
                            <span className="relative z-10 inline-flex items-center gap-1.5">
                              {active && <span className="h-1.5 w-1.5 rounded-full bg-cyan-200 shadow-[0_0_10px_rgba(165,243,252,0.9)]" />}
                              {group.name}
                            </span>
                          </button>
                        )
                        ...
                        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                          <div>
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              <span className={cn('rounded-full border px-2 py-1 text-[11px]', watchlistAccentBadgeClass(group.accent))}>
                          ...
                          <div className="flex flex-wrap items-center gap-3">
                            ...
                            <button
                              ...
                              className={cn(
                                'group relative inline-flex items-center gap-2 overflow-hidden rounded-2xl border border-[var(--input-border)] bg-[var(--input-bg)] px-4 py-2 text-sm text-theme-secondary transition-all duration-200',
                                'hover:-translate-y-0.5 hover:border-cyan-400/35 hover:text-theme-primary hover:shadow-[0_12px_24px_rgba(34,211,238,0.10)] active:scale-[0.985]',
                                animatingCollapseGroupId === group.id && 'action-button-pop border-cyan-400/40 bg-cyan-500/12 text-theme-primary'
                              )}
                            >
                              <span className="action-button-shine" />
                              <ChevronDown
                                className={cn(
                                  ...
                                  animatingCollapseGroupId === group.id && 'scale-110'
                                )}
                              />
                              <span className="relative z-10">{isCollapsed ? '展开分组' : '收起分组'}</span>
                            </button>
	          ...
	          <>
                ...
                <div className="mb-6 flex items-start justify-between gap-4">
                  <div>
                    ...
                    <div className="mt-2 text-2xl font-bold text-theme-primary">修改分组信息</div>
                    <div className="mt-2 text-sm leading-6 text-theme-secondary">
                ...
                <div className="space-y-5">
                  <label className="block">
                    <div className="mb-2 text-sm text-theme-secondary">分组名称</div>
                    <input
                  ...
                  <label className="block">
                    <div className="mb-2 text-sm text-theme-secondary">分组说明</div>
                    <textarea
                          ...
                          >
                            <div className="flex items-center justify-between gap-3">
                              <div className="flex items-center gap-3">
                                <span className={cn('h-3 w-3 rounded-full shadow-[0_0_12px_rgba(255,255,255,0.18)]', option.dot)} />
                                <span className="text-sm font-medium">{option.label}</span>
                              </div>
                              {active && <Check className="h-4 w-4 text-cyan-100" />}
                            </div>
                          </button>
                        )
                      })}
                ...
                <div className="mt-6 flex flex-wrap justify-end gap-3">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      const currentGroup = orderedWatchlistGroups.find((group) => group.id === editingGroupId)
                      if (currentGroup) {
                  ...
                  >
                    ...
                    <span className="relative z-10">{savingGroupId !== null ? '保存中...' : '保存修改'}</span>
                  </button>
                </div>
              </div>
            </div>
